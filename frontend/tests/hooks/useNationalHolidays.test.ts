...
describe('useNationalHolidays', () => {
  ...
  it('should fetch holidays successfully', async () => {
    ...
    const { result } = renderHook(() => useNationalHolidays('NL', 2025, 'EN', true));
  ...
  it('should not fetch when disabled', async () => {
    const { result } = renderHook(() => useNationalHolidays('NL', 2025, 'EN', false));
  ...
  it('should not fetch when countryCode is empty', async () => {
    const { result } = renderHook(() => useNationalHolidays('', 2025, 'EN', true));
  ...
  it('should not fetch when year is 0', async () => {
    const { result } = renderHook(() => useNationalHolidays('NL', 0, 'EN', true));
  ...
  it('should handle non-OK HTTP responses', async () => {
    ...
    const { result } = renderHook(() => useNationalHolidays('XX', 2025, 'EN', true));
  ...
  it('should cancel fetch on unmount', async () => {
    ...
    const { unmount } = renderHook(() => useNationalHolidays('NL', 2025, 'EN', true));
  ...
  it('should refetch when countryCode changes', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ country }) => useNationalHolidays(country, 2025, 'EN', true),
  ...
  it('should refetch when year changes', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ year }) => useNationalHolidays('NL', year, 'EN', true),
      {
